...
cleanup ()
{
	...
	# get rtt
	rtt=$(cat sender-ss.txt |   sed -e ':a; /<->$/ { N; s/<->\n//; ba; }' | grep "ESTAB" |  grep "unacked" | grep -oP '\brtt:.*/' | awk -F '[: /]' '{print $2}')
	
	# get cwnd, ssthresh
	cwn=$(cat sender-ss.txt |   sed -e ':a; /<->$/ { N; s/<->\n//; ba; }' | grep "ESTAB"    |  grep "unacked" | grep -oP '\bcwnd:.*(\s|$)\bbytes_acked' | awk -F '[: ]' '{print $2","$4}')

	# get bytes in flight
	bif=$(cat sender-ss.txt |   sed -e ':a; /<->$/ { N; s/<->\n//; ba; }' | grep "ESTAB"    |  grep "unacked" | grep -oP '\bunacked:.* ' | awk -F '[: ]' '{print $2}')

	# concatenate into one CSV
	paste -d ',' <(printf %s "$ts") <(printf %s "$sender") <(printf %s "$retr") <(printf %s "$rtt") <(printf %s "$cwn") <(printf %s "$bif") > sender-ss.csv
